...
export type StatusProps = ComponentProps<typeof Badge> & {
  status: 'online' | 'offline' | 'maintenance' | 'degraded' | 'sleep' | 'dnd';
};
  ...
  <span className={cn('relative flex h-2 w-2', className)} {...(props as any)}>
    <span
      className={cn(
        ...
        'group-[.sleep]:bg-yellow-500',
        'group-[.dnd]:bg-teal-500'
      )}
    />
    <span
      className={cn(
  ...
  <span className={cn('text-muted-foreground', className)} {...(props as any)}>
    {children ?? (
      <>
        ...
        <span className="hidden group-[.sleep]:block">Sleep</span>
        <span className="hidden group-[.dnd]:block">Do Not Disturb</span>
      </>
    )}
  </span>
);
